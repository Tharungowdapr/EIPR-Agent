...
export default function StrategyPage({ params }: { params: { id: string } }) {
  ...
  const runAgent = async () => {
    ...
    setProgressSteps([]);
    try {
      await agentsAPI.strategyStream(params.id, selectedOpp, (event) => {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      {opportunities.length === 0 ? (
        <PrereqCard href={`/projects/${params.id}/opportunities`} message="Complete Opportunities first" />
      ) : (
            ...
            <>
              ...
              {businessPlan.business_plan?.executive_summary && (
                <div className="card">
                  ...
                  <p className="text-sm text-[var(--text-primary)] leading-relaxed">{businessPlan.business_plan.executive_summary}</p>
                </div>
              )}
                ...
                <div className="card">
                  ...
                  {businessPlan.marketing_plan.uvp && (
                    <div className="mb-3 p-3 bg-brand-600/10 border border-brand-600/30 rounded-lg">
                      ...
                      <p className="text-sm text-[var(--text-primary)]">{businessPlan.marketing_plan.uvp}</p>
                    </div>
                  )}
              ...
              {businessPlan.growth_strategy && (
                <div className="card">
                  ...
                  <p className="text-sm text-[var(--text-primary)] mb-2">{businessPlan.growth_strategy.organic_growth}</p>
                  {businessPlan.growth_strategy.strategic_alliances?.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {businessPlan.growth_strategy.strategic_alliances.map((a: string, i: number) => (
                          <span key={i} className="badge text-xs bg-brand-600/20 text-brand-400">{a}</span>
                        ))}
                      </div>
                    </div>
                  )}
    ...
    <div className="card">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <div className="kpi-card p-3">
          <div className="kpi-value text-sm">₹{(fin.startup_costs?.total_initial_investment || 0).toLocaleString('en-IN')}</div>
          <div className="kpi-label">Initial Investment</div>
        </div>
        <div className="kpi-card p-3">
          <div className="kpi-value text-sm text-emerald-400">₹{fin.revenue_projections?.year_1?.arr?.toLocaleString('en-IN') || '0'}</div>
          <div className="kpi-label">Year 1 ARR</div>
        </div>
        <div className="kpi-card p-3">
          <div className="kpi-value text-sm">{fin.break_even_analysis?.break_even_months || '-'}mo</div>
          <div className="kpi-label">Break-even</div>
        </div>
        <div className="kpi-card p-3">
          <div className="kpi-value text-sm text-brand-400">{fin.funding_strategy?.investor_readiness_score || '-'}/10</div>
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        ...
        {fin.financial_metrics && (
          <div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between"><span className="text-[var(--text-secondary)]">Gross Margin</span><span className="text-[var(--text-primary)]">{fin.financial_metrics.gross_margin_pct}%</span></div>
              <div className="flex justify-between"><span className="text-[var(--text-secondary)]">LTV:CAC</span><span className="text-[var(--text-primary)]">{fin.financial_metrics.ltv_cac_ratio}x</span></div>
              <div className="flex justify-between"><span className="text-[var(--text-secondary)]">Year 3 ROI</span><span className="text-emerald-400">{fin.financial_metrics.roi_year_3_pct}%</span></div>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="space-y-1 text-sm">
            {fin.startup_costs.cost_breakdown.map((c: any, i: number) => (
              <div key={i} className="flex justify-between">
                ...
                <span className="text-[var(--text-primary)]">₹{(c.amount || 0).toLocaleString('en-IN')}</span>
              </div>
            ))}
          </div>
        </div>
